function [DriftX_diff,DriftY_diff,DriftZ_diff] = DriftEstimate(locTable_in,frameNum,pixelsize,frameStep,dimensional)
...
for i = 1:length(indices)
    ...
    frameSteRepY = repelem(frameStep,size(classInfo,1)-1)/pixelsize;
    frameSteRepY = cumsum(frameSteRepY);
    classInfo(:,4) = classInfo(:,4) + [0;frameSteRepY'];
    diffDriftYT = [0;diff(classInfo(:,4))];
    if strcmp(dimensional, '3D')
        frameSteRepZ = repelem(frameStep,size(classInfo,1)-1)/frameStep;
        frameSteRepZ = cumsum(frameSteRepZ);
        classInfo(:,5) = classInfo(:,5) - [0;frameSteRepZ'];
